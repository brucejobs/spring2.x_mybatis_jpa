...
public interface SqlObjectDoing<T> extends BaseDoing<T> {
    ...
    default T go(Class clazz, NamedParameterJdbcTemplate jdbcTemplate, Logger log, ObjectMapper objectMapper) {
        ...
        try {
            ...
            String dataSql = sql.toString().replace(CommonConstant.PERCENT_S.getValue(), fields);
//            if (fields.length() >= 1) {
...
//            }
            ...
            if (clazz == Long.class || clazz == Double.class || clazz == Float.class || clazz == Integer.class || clazz == String.class) {
